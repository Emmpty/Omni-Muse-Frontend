import { Currency, BountyEngagementType } from '~/types/prisma/schema';
...
export const useBountyEngagement = () => {
  ...
  const handleToggle = async (payload: any) => {
...
export const useMutateBounty = (opts?: { bountyId?: number }) => {
  ...
  const handleUpdateBounty = (data: any) => {
    if (!bountyId) return;
  ...
  const handleUpsertBounty = (data: any) => {
