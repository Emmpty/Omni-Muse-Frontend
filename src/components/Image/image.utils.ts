import { MediaType, MetricTimeframe, ReviewReactions } from '~/types/prisma/schema';
...
export const useDumbImageFilters = (defaultFilters?: Partial<any>) => {
  const [filters, setFilters] = useState<Partial<any>>(defaultFilters ?? {});
...
export const useQueryImages = (
  ...
  options?: {
    ...
    enabled?: boolean;
    applyHiddenPreferences?: boolean;
    type: number;
  }
) => {
  const { applyHiddenPreferences = true, type = 1, ...queryOptions } = options ?? {};
  filters ??= {};
  const data = { pages: [] };
  // const { data, isLoading, ...rest } = trpc.image.getInfinite.useInfiniteQuery(
  const { isLoading, ...rest } = trpc.image.getInfinite.useInfiniteQuery(
    {
      ...filters,
    },
    {
