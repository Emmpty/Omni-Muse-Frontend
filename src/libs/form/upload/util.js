...
function calculateFileHash(fileChunkList) {
  return new Promise((resolve) => {
    if (typeof window !== 'undefined') {
      // 在这里使用 worker
      const worker = new Worker(new URL('./hash.md5.worker.js', import.meta.url));
      // const worker = new Worker(new URL('./hash.sha256.worker.js', import.meta.url));
...
// 生成文件切片
function createFileChunk(file, type, blockSize = CHUNK_SIZE) {
  const fileChunkList = [];
