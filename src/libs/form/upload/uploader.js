...
import { VERIFY_FILE_API, GET_CHUNK_FILE_CID } from './api';
...
export default class Uploader {
  ...
  secondUpload = (file) => {
    ...
    if (this.customCompleteHandler) {
      this.getFileCheck_result().then((res) => {
  ...
  // 限制请求并发数
  requestWithLimit = (fileChunkList, max = MAX_REQUEST_NUM, retry = MAX_RETRY_NUM) => {
  ...
  upload = ({ fileType, file }) => {
    ...
    if (this.enableSlice) {
      // 文件切片
      const chunkList = createFileChunk(file, this.fileType);
      ...
      calculateFileHash(chunkList).then((hash) => {
        ...
        this.calHashTime = parseInt(String((hashEndTime - hashStartTime) / 10)) / 100;
        console.log(this.calHashTime, 'Time Consumption', hash);
