...
import { getUserLikes } from '~/request/api/user';
...
function Y(props: any) {
  return (
    <DatasetCard {...props} hrefLink={`/data-set/${props.info.id}/${props.info.name}?type=likes`} />
  );
}
...
function K(props: any) {
  return (
    <BountiesCard
      {...props}
      hrefLink={`/bounties/${props.info.id}/${props.info.name}?type=likes`}
    />
  );
}
